...
<form id="editForm">
  <table width="100%" border="0" cellspacing="2" cellpadding="2">
    ...
    <tr>
      ...
      <td><?= form_dropdown('fldParent',$listCategory,$mainObj?$mainObj->parent:1,'id="fldParent"');?></td>
    </tr>
  </table>
</form>
...
$(function() {
	...
	$("#buttonSave").click(function(event) {
		var resHtml = $.ajax({
			url: '<?= site_url("adminwork/subUpdate");?>',
			async: false,
			type: "POST",
			data: {
		...
		else window.location.replace('<?= site_url("adminwork/subcategories");?>');
	});
	...
	$("#buttonDelete").click(function(event) {
		var resHtml = $.ajax({
			url: '<?= site_url("adminwork/subDelete");?>',
			async: false,
			type: "POST",
			data: {
				fldId: <?= $mainObj->id;?>
		...
		alert(resHtml);
		window.location.replace('<?= site_url("adminwork/subcategories");?>');
	});
	<? endif;?>
