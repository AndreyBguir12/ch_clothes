...
$(function() {
	...
	$("#buttonSave").click(function(event) {
		var resHtml = $.ajax({
			url: '<?= site_url("adminwork/catUpdate");?>',
			async: false,
			type: "POST",
			data: {
		...
		else window.location.replace('<?= site_url("adminwork/categories");?>');
	});
	...
	$("#buttonDelete").click(function(event) {
		var resHtml = $.ajax({
			url: '<?= site_url("adminwork/catDelete");?>',
			async: false,
			type: "POST",
			data: {
				fldId: <?= $mainObj->id;?>
		...
		alert(resHtml);
		window.location.replace('<?= site_url("adminwork/categories");?>');
	});
	<? endif;?>
