...
if($listArr):
	...
	$this->table->set_heading('#','Наименование','Категория');
	foreach($listArr as $n=>$v):
		$this->table->add_row(
			$v->id,
			$v->name,
			$listCategory[$v->parent]
		);
	endforeach;
...
$(function() {
	...
	$("#buttonNew").click(function(event) {
		window.location.replace('<?= site_url("/adminwork/subForm");?>' + '/-1');
	});
	
	$("#mainTable td").on('click',function() {
		window.location.replace('<?= site_url("/adminwork/subForm");?>' + '/' + $(this).parent().children().html());
	});
